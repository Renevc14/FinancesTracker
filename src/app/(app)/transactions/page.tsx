...
import { EarnFilter } from "@/components/forms/earn-filter";
...
export default async function TransactionsPage({
  ...
  searchParams: Promise<{ earn?: string }>;
}) {
  const { earn } = await searchParams;
  const showEarn = earn === "1";
  const rows = await listTransactionsAction({ showEarn });
  const groups = groupByMonth(rows);

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <EarnFilter showEarn={showEarn} />
